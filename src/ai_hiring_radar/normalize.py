...
def _recruitee_record_value(
    ...
    *fields: str,
    ...
    for record in records:
        for field in fields:
            cleaned = _clean_optional(record.get(field))
...
def _recruitee_named_value(value: object | None) -> str | None:
    if isinstance(value, dict):
        return _clean_optional(value.get("name") or value.get("title"))
    return _clean_optional(value)
...
def _recruitee_location_display(value: object | None) -> str | None:
    if isinstance(value, dict):
        full_address = _clean_optional(
            value.get("full_address") or value.get("fullAddress")
        )
        if full_address is not None:
            return full_address

        city = _clean_optional(value.get("city"))
        country = _clean_optional(value.get("country") or value.get("country_code"))
        if city and country:
            return f"{city}, {country}"
        return _clean_optional(value.get("name") or value.get("location") or city)
    return _clean_optional(value)
...
def _recruitee_location_values_from_record(record: dict[str, Any]) -> list[str]:
    values: list[str] = []
    _append_clean_unique(values, _recruitee_location_display(record.get("location")))

    for location in _recruitee_location_items(record):
        _append_clean_unique(values, _recruitee_location_display(location))
        if not isinstance(location, dict):
            continue
        _append_clean_unique(values, location.get("full_address"))

    _append_clean_unique(values, record.get("city"))
    _append_clean_unique(values, record.get("country"))
    _append_clean_unique(values, record.get("country_code"))
    return values
    ...
    for record in records:
        for location in _recruitee_location_items(record):
            ...
            for field in ("city", "country", "country_code"):
                _append_clean_unique(values, location.get(field))
    return values
...
def _recruitee_display_location(records: list[dict[str, Any]]) -> str | None:
    for record in records:
        ...
        if location is not None:
            return location
    return _first_value(_recruitee_location_values(records))
...
def _recruitee_country_values(records: list[dict[str, Any]]) -> list[str]:
    ...
    for record in records:
        ...
        for location in _recruitee_location_items(record):
            if isinstance(location, dict):
                _append_clean_unique(values, location.get("country"))
                _append_clean_unique(values, location.get("country_code"))
                _append_clean_unique(values, location.get("full_address"))
            else:
                _append_clean_unique(values, location)
    return values
...
def _recruitee_description(records: list[dict[str, Any]]) -> str | None:
    ...
    for field in (
        ...
        for record in records:
            _append_clean_unique(values, record.get(field))
    return "\n\n".join(values) or None
...
def normalize_recruitee_offer(
    *,
    ...
    platform_job_id = listing_platform_job_id or stable_sha256(
        (SourceName.RECRUITEE.value, platform_company_slug, job_title_raw, location)
    )
    role_search_term = _ats_role_search_term(job_title_raw)
    job_title_normalized = normalize_job_title(
        job_title_raw,
    ...
    ) or _company_name_from_slug(platform_company_slug)
    company_normalized = normalize_company_name(company_raw)
    ...
    return {
        "record_type": "job_candidate",
        "job_id": stable_sha256(
            (SourceName.RECRUITEE.value, platform_company_slug, platform_job_id)
        ),
        "country_code": _first_or_empty(country_inference.country_codes),
        "country": _first_value(country_inference.countries),
        "job_country_codes": country_inference.country_codes,
...
def _html_to_text(value: object | None) -> str | None:
    ...
    text = re.sub(r"<[^>]+>", " ", text)
    return _clean_optional(unescape(text))
...
def _teamtailor_location_display(location: ET.Element) -> str | None:
    ...
    for field in ("name", "city", "country"):
        _append_clean_unique(values, _xml_child_text(location, field))
...
def _teamtailor_location_values(item: ET.Element) -> list[str]:
    ...
    for location in _teamtailor_locations(item):
        _append_clean_unique(values, _teamtailor_location_display(location))
    return values
...
def _teamtailor_country_source_locations(item: ET.Element) -> list[str]:
    values: list[str] = []
    for location in _teamtailor_locations(item):
        _append_clean_unique(values, _xml_child_text(location, "country"))
        _append_clean_unique(values, _xml_child_text(location, "city"))
        _append_clean_unique(values, _xml_child_text(location, "name"))
        _append_clean_unique(values, _teamtailor_location_display(location))
    return values
...
def _teamtailor_link_slug(source_url: str, board_url: str) -> str | None:
    ...
    return _clean_optional(parsed_url.path.rstrip("/").rsplit("/", 1)[-1])
...
def normalize_teamtailor_item(
    *,
    ...
    platform_job_id = (
        ...
        or stable_sha256(
            (SourceName.TEAMTAILOR.value, platform_company_slug, job_title_raw, location)
        )
    )
    role_search_term = _ats_role_search_term(job_title_raw)
    job_title_normalized = normalize_job_title(
    ...
    return {
        "record_type": "job_candidate",
        "job_id": stable_sha256(
            (SourceName.TEAMTAILOR.value, platform_company_slug, platform_job_id)
        ),
        "country_code": _first_or_empty(country_inference.country_codes),
        "country": _first_value(country_inference.countries),
        "job_country_codes": country_inference.country_codes,
...
def normalize_raw_ats_file(raw_file: Path) -> list[dict[str, Any]]:
    ...
    if platform == SourceName.RECRUITEE.value:
        return [
            candidate
    ...
    if platform == SourceName.TEAMTAILOR.value:
        return [
            candidate

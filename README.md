...
For a new installation, create the table and indexes by running `architecture-design-documents/04-company-review-state/setup.sql` from the Supabase SQL editor. For an existing installation created before Communication History was added, run `architecture-design-documents/04-company-review-state/migrate_add_communication_history.sql` before deploying the updated app. The migration preserves the existing `notes` values used for General Notes and initializes Communication History as empty.

...
9. Generic public emails such as `info@`, `hello@`, `contact@`, or `careers@` are allowed and stored in the same contacts list as named contacts.
10. Named-person contacts are preferred over generic emails when both are available.
11. Email addresses must not be inferred from naming patterns.
12. LinkedIn profile URLs are allowed when surfaced through search results or other public sources.
13. Every non-null factual field and every contact should include one or more source URLs where possible.
14. The persisted enrichment record must not include full page text, full search result payloads, or raw LLM responses.
15. The enrichment step must be runnable separately after `process`.
...
3. `company_size` is stored as the sourced text, for example `51-200 employees` or `1,001-5,000 employees`.
...
9. `contacts` includes named people and generic public email inboxes.
10. A generic public inbox uses `role="generic_company_email"`, `name=null`, and `title=null`.
11. Each contact should have `source_urls`.
12. `source_urls` is the de-duplicated union of field/contact source URLs used by the model.
...
Generic public company inboxes are allowed and should be included as contacts with role generic_company_email.
Prefer named-person contacts for outreach, but keep generic public emails when found.
LinkedIn profile URLs are allowed when found through web search or public source pages.
